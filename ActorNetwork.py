...
from keras.initializers import RandomNormal as normal, glorot_normal
...
class ActorNetwork(object):
    def __init__(self, sess, state_size, action_size, DDPG_config):
        ...
        DDPG_config.setdefault('minibatch_size', 64)
        self.model, self.weights, self.state = self.create_actor_network(state_size, action_size)
        self.target_model, self.target_weights, self.target_state = self.create_actor_network(state_size, action_size)
        self.action_gradient = tf.Variable(tf.zeros([DDPG_config['minibatch_size'], action_size]), dtype=tf.float32)
